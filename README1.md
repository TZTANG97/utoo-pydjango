# utoo-pydjango（愉兔 / UTOO）

**愉兔（UTOO）检测平台**全栈 monorepo：Vue3 前端 + Django 网关 + 微服务（对内中台雏形）+ 公共库 + Worker。

> 根目录 [README.md](README.md) 保持原版启动说明；**本文件**补充愉兔定位与中台抽离口径。

**当前策略（中台优先）：** 在本仓把与青岛商城共有的愉兔能力收成中台 API；`E:\qd\mall_qingdao` **暂不重构**，仅作对照。流程见 [`docs/愉兔与青岛商城中台抽离流程.md`](docs/愉兔与青岛商城中台抽离流程.md)，清单见 [`docs/中台共有能力抽离清单.md`](docs/中台共有能力抽离清单.md)。
...
能力细则见 [`docs/中台能力清单.md`](docs/中台能力清单.md)。摘要如下：

| 业务域 | 主要服务 | 说明 |
|--------|----------|------|
| 实验管理 | `qd_svc_order` | 实验主单/子单/分包、抢单、实验主数据、样品相关（共有抽离 **P0**） |
...
| 目录 | 角色 |
|------|------|
| `E:\qd\mall_qingdao` | 青岛商城旧 Spring 系统（内嵌 `utexperiment*` 愉兔业务；重构后应对接本仓中台） |
...
```env
# qd_web_front/.env.development.local
