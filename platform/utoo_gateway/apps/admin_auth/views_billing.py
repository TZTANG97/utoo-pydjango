...
from apps.core.admin_platform_forward import forward_admin_platform_first
...
def payment_apply_detail(request: Request, user=None):
    ...
    if not apply_id:
        return Response(ajax_fail("缺少 id"))
    row = billing_repo.get_payment_application_detail(int(apply_id))
    if not row:
