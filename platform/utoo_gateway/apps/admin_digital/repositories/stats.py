...
def lab_ids_for_dept(dept_id: str) -> list[int] | None:
    """部门 → 实验线所属实验室 id 列表。
    ...
    非空：按 lab_id IN (...) 过滤。
    """
    if not dept_id or dept_id in ("0", ""):
        return None
    ids = dept_lab_ids(dept_id)
    if ids:
        return ids
    rows = fetch_all(
        """
        SELECT DISTINCT el.lab_id AS lab_id
        FROM experiment_line el
        INNER JOIN experiment_order_child c ON c.line_id = el.id AND c.deleteStatus = 0
        INNER JOIN statistic_experiment_finish t ON t.child_id = c.id AND t.deleteStatus = 0
        WHERE t.test_lab = %(dept_id)s
          AND el.deleteStatus = 0
          AND el.lab_id IS NOT NULL
        """,
        {"dept_id": dept_id},
    )
    return [
        int(r["lab_id"])
        for r in rows
        if r.get("lab_id") is not None and str(r["lab_id"]).isdigit()
    ]


def list_experiment_lines(*, lab_ids: list[int] | None = None, user_id: str = "") -> list[dict[str, Any]]:
    if lab_ids is not None and len(lab_ids) == 0:
        return []
...
def dashboard_overview(
    *,
    ...
    lab_ids = lab_ids_for_dept(test_lab)
    linelist = list_experiment_lines(lab_ids=lab_ids, user_id=user_id)
