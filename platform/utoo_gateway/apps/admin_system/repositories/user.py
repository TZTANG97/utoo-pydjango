...
def list_users(
    *,
    ...
    page: int,
    page_size: int,
    management_page: bool = False,
) -> tuple[list[dict[str, Any]], int]:
    # 用户管理页对齐 Java queryUsers.do → getuserinfoMap
    # 下拉选人对齐 Java queryUsersByType → CONVERT(user_name USING GBK)
    ...
    if management_page:
        where += " AND (u.amk_type IS NULL OR u.amk_type != '小程序对外用户')"
    if dropdown_mode:
        if rt == "0":
            where += """
              AND u.utoo_type IN (
    ...
    order_sql = "ORDER BY CONVERT(u.user_name USING GBK) ASC"
    rows = fetch_all(
        f"""
