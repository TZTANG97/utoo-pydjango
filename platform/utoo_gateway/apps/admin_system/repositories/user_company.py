...
def list_companies(
    *,
    ...
    """对齐 Java QdUserCompanyMapper.listPage：含已禁用企业，不按 delete_status 过滤。"""
    where = "WHERE 1=1"
    params: dict[str, Any] = {}
    if name:
        ...
        params["name"] = f"%{name}%"
    if contract_people:
        where += " AND u.contract_people LIKE %(contract_people)s"
        params["contract_people"] = f"%{contract_people}%"
    if contract_phone:
        where += " AND u.contract_phone LIKE %(contract_phone)s"
        params["contract_phone"] = f"%{contract_phone}%"
    if company_prop:
        where += " AND u.company_prop = %(company_prop)s"
        params["company_prop"] = company_prop
    if country:
    ...
    join_sql = """
        ...
        LEFT JOIN sy_users su ON u.syuser_id = su.id
    """
    ...
    rows = fetch_all(
        ...
        SELECT
            ...
            u.company_prop, u.is_internal_account, u.syuser_id, u.delete_status,
...
def set_company_status(company_id: int, *, enabled: bool) -> None:
    """对齐 Java updateStatus.ajax：1=开启，2=禁用。"""
    execute(
        """
        UPDATE qd_user_company
        SET delete_status = %(delete_status)s, modify_time = NOW()
        WHERE id = %(id)s
        """,
        {"id": company_id, "delete_status": 0 if enabled else 1},
    )
...
def _normalize_company_list(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "company_prop": row.get("company_prop"),
        "is_internal_account": row.get("is_internal_account"),
        "syuser_id": row.get("syuser_id"),
        "user_name": row.get("user_name"),
        "deleteStatus": row.get("delete_status"),
    }
