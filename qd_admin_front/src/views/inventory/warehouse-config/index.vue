      ...
      <el-form label-width="100px">
        ...
        <template v-if="posForm.batch">
          <el-form-item label="起始编号" required>
            <el-input v-model="posForm.startNumber" placeholder="�?1" />
          </el-form-item>
          <el-form-item label="结束编号" required>
            <el-input v-model="posForm.endNumber" placeholder="�?20" />
          </el-form-item>
          <el-form-item label="字符前缀">
            <el-input v-model="posForm.charNumber" placeholder="可�? />
          </el-form-item>
        </template>
        <el-form-item v-else label="位置编号" required>
          <el-input v-model="posForm.number" placeholder="�?01" />
        </el-form-item>
      </el-form>
      <template #footer>
...
async function submitPos() {
  ...
  if (posForm.batch) {
    if (!posForm.startNumber || !posForm.endNumber) {
      ElMessage.warning('请填写起止编�?)
      return
    }
  } else if (!posForm.number.trim()) {
    ElMessage.warning('请填写位置编�?)
    return
  }
  saving.value = true
  try {
