...
from apps.identity.models import ExpUser
...
class CustomerUserService:
    ...
    def rehash_password_if_legacy(user: ExpUser, plain: str) -> None:
        ...
        if not should_rehash_to_java(stored):
            return
        hashed = encrypt_password_for_storage(plain)
        ExpUser.objects.filter(pk=user.pk).update(password=hashed)
    ...
    @staticmethod
    def exp_user_token_data(user: ExpUser, *, channel: str) -> dict:
        return {
            "sub": str(user.id),
            "user_id": str(user.id),
