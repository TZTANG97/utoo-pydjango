    ...
    if int(bill_type or 2) == 2:
        ...
        try:
            ...
            order_repo._write_order_log(
                int(order_id),
                f"收款 {float(money)}" + (f"：{log_info}" if log_info else ""),
                user_id=uid,
            )
            order_repo.auto_generate_appointment_after_online_pay(
                order_id=int(order_id), staff_user_id=uid
            )
            order_repo.try_finish_main_order(order_id=int(order_id), staff_user_id=uid)
        except Exception:
            logger.exception("post online receive hooks order_id=%s", order_id)
