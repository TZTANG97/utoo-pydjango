...
export default {
        ...
        if (visible) {
          ...
          setTimeout(() => {
            this.integral_convert_ratio =
              this.$store.getters.IntegralConvertRatio;
            this.totalIntegral = this.$store.getters.totalIntegral;
          }, 1000);
        }
      },
    },
  ...
  methods: {
    ...
    // 校验输入的积分和实际积分
    inputChange(val) {
      this.integral_num = val.replace(/[.]+/g, "");
      let data = new Decimal(
        this.integral_num == "" ? 0 : this.integral_num
      ).div(new Decimal(100).div(this.integral_convert_ratio));
