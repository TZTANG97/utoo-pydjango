	...
	<view class="container">
		<view class="group">
			<view class="group-item" v-if="id">
				<view class="group-label"> 订单编号： </view>
			...
			<view class="group-item" v-if="id">
				<view class="group-label"> 制单人员： </view>
				<view class="group-content">
					<view class="text-content">
						{{
              detail.mainData.addUser.trueName
                ? detail.mainData.addUser.trueName
                : "暂无"
            }}
					</view>
			...
			<view class="group-item">
				<view class="group-label"> 下单时间： </view>
				<view class="group-content">
			...
			<view class="group-item">
				<view class="group-label"> 客户名称： </view>
				<view class="group-content">
			...
			<view class="group-item">
				<view class="group-label"> 销售主管： </view>
				<view class="group-content">
					<view class="text-content" @click="
              select('userName', 'saleBossList', '销售主管', 'purchaseBossId')
            ">
			...
			<view class="group-item">
				<view class="group-label"> 销售人员： </view>
				<view class="group-content">
					<view class="text-content" @click="
              select(
                'userName',
			...
			<view class="group-item">
				<view class="group-label"> 供应商： </view>
				<view class="group-content">
					<view class="text-content" @click="
              select('company_name', 'companyList', '供应商', 'companyId')
            ">
			...
			<view class="group-item">
				<view class="group-label"> 订单币种： </view>
				<view class="group-content">
			...
			<view class="group-item">
				<view class="group-label"> 分成信息： </view>
			...
			<view class="group-item">
				<view class="group-label"> 预计收货时间： </view>
				<view class="group-content">
			...
			<view class="group-item">
				<view class="group-label"> 付款方式： </view>
				<view class="group-content">
			...
			<view class="group-item" v-for="(item, index) in payDateList" :key="index">
				<view class="group-label"> 预计收款时间： </view>
			...
			<view class="group-item">
				<view class="group-label"> 订单总价： </view>
				<view class="group-content">
					<view class="text-content">
			...
			<view class="group-item">
				<view class="group-label"> 是否开票： </view>
				<view class="group-content">
					<view class="text-content">
			...
			<view class="group-item" v-if="invoiceType">
				<view class="group-label"> 出项开票类型： </view>
				<view class="group-content">
			...
			<view class="group-item" v-if="invoiceType">
				<view class="group-label"> 税率： </view>
				<view class="group-content">
			...
			<view class="group-item">
				<view class="group-label"> 订单资料： </view>
				<view class="group-content">
			...
			<view class="files-list" v-if="files.length">
				<view class="file-item" v-for="(item, index) in files" :key="index">
					...
					<image src="@/static/delete.png" mode="" @click="deleteFile(item.id, index)"></image>
				</view>
			</view>
			<upload-progress ref="prg"></upload-progress>

			<view class="group-item" style="align-items: start !important">
				<view class="group-label"> 订单备注： </view>
				<view class="group-content">
					<view class="text-content">
		...
		<view class="group child-order" v-for="(item, index) in childOrderList" :key="index">
			...
			<view class="group-item">
				<view class="group-label"> 产品名称： </view>
				<view class="group-content">
			...
			<view class="group-item">
				<view class="group-label"> 产品型号： </view>
				<view class="group-content">
					<view class="text-content" @click="chooseProductModel(item.newAdd, index, item.goodsSpecList)">
			...
			<view class="group-item">
				<view class="group-label"> 产品品牌： </view>
				<view class="group-content">
					<view class="text-content">
			...
			<view class="group-item">
				...
				<view class="group-content">
					<view class="text-content">
						<input :disabled="isNum" @input="getTotal(true)" class="child-order-input" type="digit"
							v-model.number.lazy="item.goodsNums" placeholder="请输入数量" />
					</view>
				</view>
			</view>
			<view class="group-item">
				<view class="group-label"> 实验测试项目： </view>
			...
			<view class="group-item">
				<view class="group-label"> 实验测试分类： </view>
				<view class="group-content">
					<view class="text-content">
						{{
			...
			<view class="group-item">
				<view class="group-label"> 实际测试金额： </view>
				<view class="group-content">
					<view class="text-content">
			...
			<view class="group-item">
				<view class="group-label"> 标准测试金额： </view>
				<view class="group-content">
					<view class="text-content">
			...
			<view class="group-item">
				<view class="group-label"> 总价： </view>
				<view class="group-content">
					<view class="text-content">
		...
		<view class="confirm" @click="submitData"> 确&nbsp;定 </view>
		...
		<!-- 分页加载-获取实验项目 -->
		<popup-bottom :loadable="true" :loading="testLoading" :is-refresh.sync="testIsRefresh"
		...
		<!-- 添加子订单图标 -->
		<view class="add-order" @click="
        (showDynamicPopup = true),
          (currentSelectedChildOrderIndex = childOrderList.length)
      ">
			<image src="@/static/add.png" mode=""></image>
		</view>

		<!-- 分成信息 -->
		<devide-into :lr="lr" :showLr="true" :backUpList="lrList" :show.sync="showDevideInfo"
			@confirmDevideList="confirmDevideInfo"></devide-into>
	...
	export default {
		...
		methods: {
			...
			confirmDevideInfo({
				...
				if (total !== 100) {
					return this.$tip("利润分成比例总和必须是100%");
				}
				this.lr = lr
			...
			confirmTestProject(e) {
				...
				this.childOrderList[this.currentSelectedChildOrderIndex][
					"experiment_project_name"
				] = project_name;
				this.childOrderList[this.currentSelectedChildOrderIndex][
					"experiment_class_name"
				] = class_name;
				this.childOrderList[this.currentSelectedChildOrderIndex]["goodsPrice"] =
					test_price;
				this.childOrderList[this.currentSelectedChildOrderIndex][
					"reference_price"
				] = test_price;
			...
			submitData() {
				...
				if (this.id) {
					saveTestSubOrderApi(arrData).then((res) => {
						if (res.res) {
							uni.$emit('isEdit');
							uni.redirectTo({
					...
					if (this.disabledTotalPrice != this.totalPrice) {
						...
						uni.showModal({
							...
							success({
								...
								if (confirm) {
									addTestSubOrderApi(arrData).then((res) => {
										if (res.res) {
											uni.redirectTo({
			...
			deleteChildOrder(index) {
				if (this.childOrderList.length == 1)
					return this.$tip("最后一个子订单不能删除");
				let that = this;
				uni.showModal({
					title: "提示",
					content: "您确定删除该订单吗？",
			...
			chooseProductModel(
				isNew,
				currentSelectedChildOrderIndex,
				_productModelList
			) {
				if (!isNew) return;
				this.productModelList = []
				_productModelList.forEach(item=>{
					if(item)this.productModelList.push(item)
				})
			...
			getDetail() {
				editTestSubOrderApi({
					...
					if (res.res) {
						...
						if(this.detail.order_status == 5 || this.detail.order_status == 10 || this.detail.order_status == 20){
							this.isNum = true
						}else{
							this.isNum = false
						}
			...
			getpurchasePersonnelList(type) {
				fetchBunListApi({
					...
					if (res.res) {
						// 采购人员,分成人员
						if (type == -1) {
							this.purchasePersonnelList = this.lrList = res.obj;
						} else {
