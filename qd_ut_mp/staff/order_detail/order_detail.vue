<template>
	<view class="container" v-if="my_data">
		<view class="group">
			...
			<view class="group-item" @click="showDevide = true" v-if="roleName!='R类人员' && flag">
				<view class="group-label">
					分成信息
				</view>
				<view class="group-content">
		...
		<view class="detail-box" v-if="roleName!='R类人员'">
			<text>历史操作记录</text>
			<!-- <text style="color: #f39800;font-weight: 700;padding: 10rpx 0;font-size: 25rpx;">测试进度请点击子订单进入查看</text> -->
		...
		<u-popup mode="right" width="60%" v-model="showMenus">
			<view class="menus-list" v-if="my_data">
				<navigator :url="`/staff/edit_order/edit_order?id=${id}`" hover-class="none" class="btn"
					v-show="(my_data.order_status === 5 || my_data.order_status === 10 || my_data.order_status === 20) && syUser.userType !== '系统管理员' && isDisabled && !isType && roleName != 'R类人员' && roleName != 'H类用户'">
					编辑订单
				</navigator>

				<navigator :url="`/staff/edit_order/edit_order?id=${id}`" hover-class="none" class="btn"
					v-show="my_data.order_status > 0 && syUser.userType === '系统管理员' && isDisabled && !isType && roleName != 'R类人员' && roleName != 'H类用户'">
					编辑订单
				</navigator>

				<view class="btn" @click="cancelOrder(2)"
					v-show="(my_data.order_status === 5 || my_data.order_status === 10 || my_data.order_status === 20 || my_data.order_status === 30) && !isType && roleName != 'R类人员' && roleName != 'H类用户'">
					取消订单
				</view>

				<view class="btn"
					v-show="my_data.order_status === 20 && !isType && roleName != 'R类人员' && roleName != 'H类用户'"
					@click="cancelAudit">
					取消审核申请
				</view>

				<view class="btn"
					v-show="my_data.order_status === 5 && !isType && roleName != 'R类人员' && roleName != 'H类用户'"
					@click="submitAudit">
					提交审核
				</view>
				<navigator :url="`/staff/add_em_sub_order/add_em_sub_order?id=${id}&class_id=${my_data.class_id}`"
					hover-class="none" class="btn"
					v-show="my_data.order_status != 0 && isOut && !isType && roleName != 'R类人员' && roleName != 'H类用户'">
					创建实验子订单
				</navigator>
				...
				<template
					...
					<view class="btn" v-show="viewKpBtn && my_data.is_online == 0" @click="setKeyWord('开票')">
						开票
					</view>
					<view class="btn" v-show="viewReciveBtn" @click="setKeyWord('收款')">
						收款
					</view>
				</template>
				...
				<view class="btn" @click="adjustDevide"
					v-show="(my_data.order_status === 67 || my_data.order_status === 5 || my_data.order_status === 20 || my_data.order_status === 30) && (syUser.userId === my_data.saleUser.id || syUser.userId === my_data.addUser.id) && isfcbl && !isType && roleName != 'R类人员' && roleName != 'H类用户' && flag">
					调整分成比例
				</view>

				<view v-show="my_data.cost_settle == 0 && !isType && roleName != 'R类人员' && roleName != 'H类用户'"
					class="btn" @click="settlementCost">
					所有成本已结清
				</view>
...
appointment relatedOrders
				...
				<navigator v-if="!isType" :url="`/staff/child_order/child_order?id=${id}&type=6`" hover-class="none"
					class="btn">
					子订单
				</navigator>
