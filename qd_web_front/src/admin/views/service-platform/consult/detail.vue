  ...
  <div v-loading="loading" class="edit-page">
    ...
    <el-form v-if="form.id" label-width="130px" class="form-card" :disabled="readonly" @submit.prevent>
      ...
      <el-table :data="childs" border stripe>
        ...
        <el-table-column label="实验项目" min-width="120">
          <template #default="{ row }">
            <el-input v-model="row.experiment_project_name" :disabled="readonly" size="small" />
          </template>
        </el-table-column>
...
import { fetchManageOptions, fetchExpGoodsList } from '@admin/api/experiment'
import {
  cancelConsult,
  getConsultDetail,
  saveConsultOrder,
...
async function searchGoods() {
  ...
  try {
    const res = await fetchExpGoodsList({
      ...
      length: 50,
      draw: 1,
      goods_name: goodsKeyword.value.trim(),
    })
    goodsRows.value = Array.isArray(res.data) ? res.data : []
  } finally {
...
function pickGoods(row: Record<string, unknown>) {
  ...
  target.goods_id = row.id
  target.goods_name = row.goods_name || row.goodsName || ''
  target.goods_spec = row.goods_model || row.goods_spec || row.goodsModel || ''
  target.goods_brand_name = row.goods_brand_name || row.goodsBrandName || row.brand_name || ''
  target.goods_brand_id = row.goods_brand_id || row.goodsBrandId || ''
...
async function loadDetail() {
  ...
  try {
    ...
    form.order_num = String(consult.order_num || '')
    form.class_id = (consult.class_id as string | number) || ''
    form.userName = String(consult.userName || '')
    form.mobile = String(consult.mobile || '')
    form.company_name = String(consult.company_name || '')
    ...
    form.supplier_name = (consult.supplier_name as string | number) || ''
    form.sale_manager = (consult.sale_manager as string | number) || ''
    form.delivery_time_str = String(consult.delivery_time_str || '').slice(0, 10)
    form.collection_time_str = String(consult.collection_time_str || '').slice(0, 10)
    form.order_type = (consult.order_type as number) || 2
    form.send_address = String(consult.send_address || '')
    form.reverso_context = flagOn(consult.reverso_context)
    form.addressee_name = String(consult.addressee_name || '')
    form.addressee_mobile = String(consult.addressee_mobile || '')
    form.test_address_id = (consult.test_address_id as string | number) || ''
    form.company_account_id = (consult.company_account_id as string | number) || ''
    form.invoiceType = flagOn(consult.invoiceType)
    form.is_video = flagOn(consult.is_video)
    form.is_arrive = flagOn(consult.is_arrive)
    form.is_on = flagOn(consult.is_on)
    ...
    childs.value = rawChilds.length
      ? rawChilds.map((c) => ({
          ...emptyChild(),
          ...c,
          goods_name: c.goods_name || (c.expGoods as Record<string, unknown> | undefined)?.goods_name || '',
          goods_spec:
            c.goods_spec || (c.expGoods as Record<string, unknown> | undefined)?.goods_model || '',
          goods_nums: c.goods_nums != null ? String(c.goods_nums) : '1',
          goods_price: c.goods_price != null ? String(c.goods_price) : '0',
          reference_price: c.reference_price != null ? String(c.reference_price) : '0',
          sample_id: c.sample_id ?? '',
        }))
      : [emptyChild()]
