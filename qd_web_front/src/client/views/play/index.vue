<template>
  <div class="container">
    <div class="operation-panel">
      <img v-if="login_way" @click="changeLoginWay(false)" class="qrcode-login" src="@client/static/qrcode.png" alt="">
      <img v-else @click="changeLoginWay(true)" class="account-login" src="@client/static/form-login.png" alt="">
      ...
      <div class="input-login" v-if="login_way">
        ...
        <el-form :model="form" status-icon :rules="rules" ref="form">
          <el-form-item prop="mobile">
            <el-input prefix-icon="el-icon-user" v-model="form.mobile" maxlength="11" autocomplete="off"
                      placeholder="请输入手机号"></el-input>
          </el-form-item>
          <el-form-item prop="nick_name" v-if="optionIndex === 1">
            <el-input prefix-icon="el-icon-edit" v-model="form.nick_name" maxlength="11"
                      placeholder="请输入昵称"></el-input>
          </el-form-item>
          <el-form-item prop="pwd">
            <el-input show-password prefix-icon="el-icon-lock" v-model="form.pwd" maxlength="20"
                      :placeholder="`请输入${optionIndex === 2? '新' : ''}密码`"></el-input>
          </el-form-item>
          <el-form-item prop="confirm_pwd" v-if="optionIndex">
            <el-input show-password prefix-icon="el-icon-lock" v-model="form.confirm_pwd" maxlength="20"
                      :placeholder="`请再次输入${optionIndex === 2? '新' : ''}密码`"></el-input>
          </el-form-item>
          <el-form-item prop="code" v-if="optionIndex">
            <el-input style="width: 260px !important" v-model="form.code" maxlength="6" prefix-icon="el-icon-key"
                      placeholder="请输入验证码"></el-input>
            <el-button class="get-note-code-btn"
                       :style="{ 'cursor': countDown === '获取验证码'? 'pointer' : 'default' }" type="text"
                       @click="getNoteCode">{{ countDown }}
            </el-button>
          </el-form-item>
          <el-form-item>
            <div class="slider" @click="verified = true" :class="{ 'pass': verified }">
          ...
          <el-form-item>
            <el-button type="primary" @click="perAction" style="width: 100%" :loading="logining">
              <span v-if="!optionIndex">登&nbsp;录</span>
              <span v-else-if="optionIndex === 1">注&nbsp;册</span>
              <span v-else>保&nbsp;存</span>
            </el-button>
            <div class="other-handle">
              <el-button type="text" @click="change(2)" v-if="optionIndex!==2">
                忘记密码
              </el-button>
              <el-button type="text" @click="change(0)" v-if="optionIndex">
                已有帐号？去登录
              </el-button>
              <el-button type="text" @click="change(1)" v-if="optionIndex!==1">
                没有账号？去注册
              </el-button>
            </div>
          </el-form-item>
      ...
      <div class="qrcode-login-wrapper" v-else>
        <div class="wx-qrcode" v-loading="loadingQrcode" element-loading-background="rgba(0, 0, 0, 0.75)">
          <img v-if="ticket" :src="ticket" alt="">
          <div class="shadow" v-if="qrcodeStatus !== 'NOT_SCAN'">
            <div class="expired" v-if="qrcodeStatus === 'EXPIRED' || qrcodeStatus === 'ERROR'">
              <span>{{ qrcodeStatusList[qrcodeStatus] }}</span>
              <div class="btn" @click="getQrcode">刷新</div>
            </div>
            <div class="scaning" v-if="qrcodeStatus === 'SCANNED'">
              <svg-icon icon-class="success" class-name="success-svg"></svg-icon>
              <span>{{ qrcodeStatusList[qrcodeStatus] }}</span><br>
              <span>请在手机端完成登录</span>
            </div>
          </div>
        </div>
        <div class="tip">打开手机 <span>微信</span> 扫描二维码登录或注册
        </div>
      </div>
    </div>
  </div>
...
import {getQrcodeApi, getQrcodeStatusApi} from "@client/api/index";
import {setToken} from '@client/utils/auth'
...
export default {
  ...
  methods: {
    ...
    check() {
      ...
      getQrcodeStatusApi(ticket).then(res => {
        this.qrcodeStatus = res.obj

        if (this.qrcodeStatus === 'EXPIRED' || Object.prototype.toString.call(res.obj) === '[object Object]') {
          clearInterval(this.checkTimer)
        }
    ...
    getQrcode() {
      ...
      getQrcodeApi()
        .then(res => {
          ...
          this.loadingQrcode = false
          this.qrcodeStatus = 'ERROR'
          this.$message.error(res?.resMsg || res?.message || '二维码加载失败')
        })
        .catch(() => {
          this.loadingQrcode = false
          this.qrcodeStatus = 'ERROR'
    ...
    getNoteCode() {
      ...
      getNoteCodeApi(this.form.mobile).then(res => {
        if (res.res) {
          let time = 60;
