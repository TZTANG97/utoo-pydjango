...
from shared.utoo_billing import repo as billing_repo
from shared.utoo_billing.db import execute, execute_insert, fetch_all, fetch_one
...
def _user_account_get_or_create(user_id: int) -> dict[str, Any]:
    """内联 user_account get_or_create，避免硬依赖 apps.payments。"""
    row = fetch_one(
        """
...
def _gen_code(seq: int) -> str:
    if seq < 100000:
        return str(seq).zfill(5)
    return str(seq)


def _payment_pa_num_generate(pay_type: str) -> str:
    """对齐 payment.pay_order_code.payment_pa_num_generate（内联，无硬依赖）。"""
    ...
    if row and row.get("pa_num"):
        ...
        return orderstr + _gen_code(n)
    return orderstr + _gen_code(1)


def _insert_receive_bill(
    *, order_id: int, money: Decimal, user_id: int, log_info: str, bill_type: int = 2
) -> None:
    """内联 INSERT qd_bill；分钱可选 import，失败只打日志。"""
    execute(
        """
        INSERT INTO qd_bill
    ...
    if int(bill_type or 2) == 2:
        # TODO(scheme-b)+2026-09-04: split_money 仅 payment 进程可用；其它中台软跳过
        try:
            ...
            logger.exception(
                "split after insert_receive_bill soft-skip order_id=%s", order_id
            )


def _order_mid_base() -> str:
    import os
...
def _try_finish_after_invoice(
    *,
    ...
    """开票后主单完成判定：本进程可 import 则本地；否则 HTTP 调 order 中台。"""
    # 1) 同进程（order 仓内）直接调
    try:
        ...
        order_repo.try_finish_main_order(
            order_id=order_id, staff_user_id=staff_log_uid
        )
        return
    except Exception:
        pass

    # 2) 跨进程：platform → order
    if not token:
        logger.warning(
            "try_finish after agree_invoice skipped (no token) order=%s", order_id
        )
        return
    try:
        import httpx

        url = f"{_order_mid_base()}/api/adminExperiment/order/tryFinishAfterInvoice.ajax"
        r = httpx.post(
            url,
            headers={
            ...
            json={
                "orderId": order_id,
                "amount": amount,
                "staffUserId": staff_log_uid,
                "logInfo": f"开票 {amount}：同意开票申请",
            },
            timeout=15,
            trust_env=False,
        )
        if r.status_code >= 400:
            logger.warning(
                "try_finish HTTP failed order=%s status=%s body=%s",
                order_id,
                r.status_code,
                (r.text or "")[:200],
            )
    except Exception:
        logger.exception("try_finish HTTP soft-fail order=%s", order_id)
...
def reject_invoice(*, apply_id: int, staff_user_id: str) -> tuple[bool, str]:
    ...
    if money > 0:
        _user_account_get_or_create(int(row["user_id"]))
        execute(
            """
            UPDATE user_account
...
def get_invoice_open_preview(apply_id: int) -> dict[str, Any] | None:
    ...
    related = {
        str(o.get("id")): o
        for o in billing_repo.list_invoice_related_orders(str(row.get("order_ids") or ""))
    }
    ...
    try:
        for oid, amt, mk, acc_id in bill_items:
            ...
            try:
                ...
                pass
            _try_finish_after_invoice(
                order_id=oid,
                amount=float(amt),
                staff_log_uid=staff_log_uid,
                token=token,
            )
...
def agree_payment(*, apply_id: int, staff_user_id: str) -> tuple[bool, str]:
    ...
    order_type = str(
        row.get("orderType") if row.get("orderType") is not None else row.get("order_type") or ""
    )
    ...
    staff = _safe_staff_uid(staff_user_id)
    order_ref = str(
        row.get("orderId") if row.get("orderId") is not None else row.get("order_id") or ""
    )
    ...
    if order_type in ("2", "3"):
        ...
        account = _user_account_get_or_create(user_id)
        execute(
            """
            UPDATE user_account
    ...
    pa_num = _payment_pa_num_generate(str(pay_type))
...
def refuse_payment(*, apply_id: int, staff_user_id: str, mark: str = "") -> tuple[bool, str]:
    ...
    if order_type in ("2", "3"):
        ...
        money = Decimal(str(row.get("money") or 0))
        uid_raw = row.get("userId") if row.get("userId") is not None else row.get("user_id")
        _user_account_get_or_create(int(uid_raw))
        execute(
            """
            UPDATE user_account
            SET amount = COALESCE(amount, 0) + %(m)s, update_time = NOW()
