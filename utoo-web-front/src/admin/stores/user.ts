...
import type { AdminMenuItem, LoginResult, WelcomeData } from '@admin/types/admin'

import { getToken, removeToken, setToken } from '@admin/utils/auth'

import { ajaxErrorMessage, isAjaxOk } from '@admin/utils/request'
import { indexPendingMenus } from '@admin/utils/menu-route'
...
const HIDDEN_MENU_NAMES = new Set(['微信支付测试'])

function isHiddenAdminMenu(item: {
  menu_name?: string
  menu_url?: string
  name?: string
  url?: string
  ...
  if (HIDDEN_MENU_NAMES.has(name)) return true
  const url = (item.menu_url || item.url || '').toLowerCase()
  return url.includes('wxpaytest')
}

function filterHiddenMenus(items: AdminMenuItem[]): AdminMenuItem[] {
  return items
    .filter((item) => !isHiddenAdminMenu(item))
    .map((item) => ({
      ...item,
      children: item.children?.length ? filterHiddenMenus(item.children) : [],
    }))
}
...
function normalizeMenus(raw: unknown): AdminMenuItem[] {
  ...
  if ('childMenu' in first || 'menuName' in first) {
    ...
    return filterHiddenMenus(
      raw.map((item) => normalizeJavaTopMenu(item as Record<string, unknown>))
    )

  }

  return filterHiddenMenus(buildMenuTree(raw as AdminMenuItem[]))
