import type { AdminMenuItem } from '@admin/types/admin'
...
function withAdminPrefix(path: string): string {
  if (!path) return path
...
export function resolveMenuPath(item: AdminMenuItem): string {
  ...
  if (raw.startsWith('#')) {
    ...
    return withAdminPrefix(hashPath.startsWith('/') ? hashPath : `/${hashPath}`)
  }
