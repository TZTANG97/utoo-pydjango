<template>
  <admin-page-card :title="pageTitle">
    <el-form :inline="true" class="filter-form" @submit.prevent>
      ...
      <el-form-item>
        <el-select v-model="filters.order_status" clearable placeholder="全部订单状态" style="width: 150px">
    ...
    <el-table v-loading="loading" :data="rows" border stripe>
      ...
      <el-table-column label="客户企业名称" min-width="160" show-overflow-tooltip>
        <template #default="{ row }">
          {{ row.customerName || row.company?.name || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="所属公司名称" min-width="160" show-overflow-tooltip>
        <template #default="{ row }">
          {{ row.supplierName || row.supplierUser?.company_name || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="销售主管" min-width="100" show-overflow-tooltip>
        <template #default="{ row }">
          {{ row.managerTrueName || row.saleManagerUser?.userName || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="销售人员" min-width="100" show-overflow-tooltip>
        <template #default="{ row }">
          {{ row.saleUserTrueName || row.saleUser?.userName || '-' }}
        </template>
      </el-table-column>
...
import { fetchDigitalCompanySaleList, fetchDigitalExpList } from '@admin/api/fund'
import { useDataTable } from '@admin/composables/useDataTable'

const route = useRoute()
const router = useRouter()

const mode = computed(() => String(route.query.mode || 'exp'))
const pageTitle = computed(() => String(route.query.title || route.meta?.title || '订单明细'))
...
function listParams() {
  ...
  if (mode.value === 'company') {
    return {
      ...
      order_type: String(q.order_type || '3'),
      account_type: String(q.account_type || '1'),
    }
  }
  return {
    ...base,
    type: String(q.type || '0'),
    currency_type: String(q.currency_type || '1'),
    company_id: String(q.company_id || ''),
    year: String(q.year || ''),
    test_type: String(q.test_type || ''),
    sale_user: String(q.sale_user || ''),
    order_type: String(q.order_type || ''),
...
const { loading, rows, total, pagination, load } = useDataTable((params) =>
  mode.value === 'company'
    ? fetchDigitalCompanySaleList({ ...params, ...listParams() })
    : fetchDigitalExpList({ ...params, ...listParams() })
)
...
function viewOrder(row: Record<string, unknown>) {
  ...
  router.push({
    ...
    query: {
      from: 'digital-center',
      orderNo: String(row.orderId || row.order_id || ''),
    },
  })
}
