import {loginApi, getInfo} from '@client/api/user'
import {getToken, setToken, removeToken} from '@client/utils/auth'
import { getIntegralConvertRatio,getIntegralApi } from "@client/api/integral";
        ...
        if (response.res) {
          ...
          getIntegralConvertRatio()
            .then(res => {
              if (res.res) {
                const raw =
                  res.obj?.integral_convert_ratio ??
                  res.obj?.integralConvertRatio
                const ratio = Number(raw)
                commit(
                  'SET_IntegralConvertRatio',
      ...
      getIntegralConvertRatio().then(res => {
        if (res.res) {
          const raw =
            res.obj?.integral_convert_ratio ??
            res.obj?.integralConvertRatio ??
            res.data?.integral_convert_ratio
          const ratio = Number(raw)
          commit(
            'SET_IntegralConvertRatio',
            Number.isFinite(ratio) && ratio > 0 ? ratio : 2
