    ...
    <div class="operation-panel">
      ...
      <div class="input-login" v-if="login_way">
        ...
        <el-form :model="form" status-icon :rules="rules" ref="form">
          ...
          <el-form-item v-if="optionIndex === 0" class="remember-row">
...
export default {
  ...
  data() {
    ...
    return {
      rememberStorageKey: "utoo-client-remember-login",
      form: {
        mobile: "",
        nick_name: "",
        pwd: "",
  ...
  methods: {
    restoreRememberedLogin() {
      try {
        const raw = localStorage.getItem(this.rememberStorageKey);
        if (!raw) return;
        const saved = JSON.parse(raw);
        this.form.mobile = saved?.mobile || "";
        this.form.pwd = saved?.pwd || "";
        this.rememberPassword = !!(saved?.mobile && saved?.pwd);
      } catch (_) {
